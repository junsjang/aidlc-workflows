...
Every applicable rule appears in `rules_in_context`. Org, team, and project rules concatenate; nothing drops at runtime. The phase rule attaches when the rule's filename matches the stage's `phase:` declaration - no glob filter, no concrete-path synthesis. Delivery is content, not paths: at directive-build time the engine reads each substantive rule file from the ACTIVE space and bakes its text into the run-stage directive's `rules_content` field (empty-template placeholder files are dropped; files past the directive size budget are named in `rules_content_omitted` for the conductor to read by path). Re-read every stage, so a learning admitted mid-workflow reaches the very next stage. The paths in `rules_in_context` remain for reference and for the omitted-file fallback.
